...
test('create preserves decimal strings, converts missing component to null and includes provenance', () => {
  assert.deepEqual(normalizePublicModelCreateForm(valid), { upstream_model_id:'up/model', model_key:'model-key', display_name:'Model', provider:'Provider', capabilities:['chat'], context_window:128000, input_price_usd_per_million_tokens:'1.25000000', output_price_usd_per_million_tokens:null, public_display_group:'general', endpoint_types:['chat'], public_restrictions:[], price_source:'catalog', price_reviewer:'root', price_effective_at:1720000000000 })
})
test('create must come from an observed upstream id and rejects unsupported prices/currency', () => {
  for (const change of [{upstreamModelId:''},{inputPrice:'01'},{outputPrice:'1.123456789'},{perCallPrice:'1'},{currency:'CNY'},{unit:'token'},{priceSource:''},{priceReviewer:''},{priceEffectiveAt:null}]) assert.throws(() => normalizePublicModelCreateForm({...valid,...change}), /invalid_public_model_form/)
})
test('update is sparse, immutable identities are forbidden and revision is required', () => {
  assert.deepEqual(normalizePublicModelUpdateForm({ expectedRevision:3, inputPrice:null, displayName:'New' }), { expected_revision:3, input_price_usd_per_million_tokens:null, display_name:'New' })
  assert.throws(() => normalizePublicModelUpdateForm({ expectedRevision:3, modelKey:'changed' }), /invalid_public_model_form/)
  assert.throws(() => normalizePublicModelUpdateForm({ expectedRevision:0 }), /invalid_public_model_form/)
})
