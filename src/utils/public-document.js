...
export function controlledAssetSrc(value) { if (typeof value !== 'string' || value !== value.trim() || /%(?![0-9a-f]{2})/i.test(value)) return false; let normalized=value.normalize('NFKC'),stable=false;const limit=Math.min(value.length+1,64);for(let i=0;i<limit;i++){let next;try{next=decodeURIComponent(normalized)}catch{return false}next=next.normalize('NFKC');if(next===normalized){stable=true;break}normalized=next}if(!stable)return false;normalized=normalized.replace(/[\p{Z}\p{Cc}\p{Cf}]/gu,''); return /^\/assets\/[A-Za-z0-9._/-]+$/.test(normalized) && !normalized.includes('//') && !normalized.endsWith('/') && normalized.split('/').every(segment => segment !== '.' && segment !== '..') }
