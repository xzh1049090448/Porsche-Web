...
export function controlledAssetSrc(value) { if (typeof value !== 'string' || value !== value.trim()) return false; let normalized=value.normalize('NFKC'); for(let i=0;i<8;i++){let next;try{next=decodeURIComponent(normalized)}catch{return false}if(next===normalized)break;normalized=next.normalize('NFKC')} normalized=normalized.replace(/[\p{Z}\p{Cc}\p{Cf}]/gu,''); return /^\/assets\/[A-Za-z0-9._/-]+$/.test(normalized) && !normalized.includes('//') && !normalized.endsWith('/') && normalized.split('/').every(segment => segment !== '.' && segment !== '..') }
