...
const visible = (overrides = {}) => ({ model_key: 'stable-key', display_name: 'Model', provider: 'Acme', capabilities: ['chat'], context_window: 128000, input_price_usd_per_million_tokens: '0.00000001', output_price_usd_per_million_tokens: '999999999999.99999999', price_visibility: 'visible', release_version: 7, ...overrides })

test('projects only exact safe model fields and retains decimal strings', () => {
  const model = mapPublicModel(visible())
  assert.deepEqual(model, { modelKey: 'stable-key', displayName: 'Model', provider: 'Acme', capabilities: ['chat'], contextWindow: 128000, inputPrice: '0.00000001', outputPrice: '999999999999.99999999', priceVisibility: 'visible', releaseVersion: 7 })
...
test('redacted and missing prices are omitted and never labelled free', () => {
  const redacted = mapPublicModel({ model_key: 'redacted', display_name: 'Hidden', provider: 'Acme', capabilities: [], context_window: 1, price_visibility: 'authenticated_only', release_version: 7 })
