...
let active = true
const load = async () => { await ready; if (active) return loadVerifiedPublicPage(state, 'about'); return null }; onMounted(() => load().catch(() => {})); onUnmounted(() => { active = false; state.invalidatePage('about') })
</script>
