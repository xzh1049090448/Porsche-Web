...
<template><article class="public-document"><header class="public-document__header"><h1>{{ content?.title || t(page) }}</h1><p v-if="content?.valid" class="public-document__meta">{{ t('version') }} {{ content.version }} · {{ t('effectiveDate') }} {{ content.effectiveDate }}</p></header><PublicContentState v-if="slot.status !== 'ready'" :status="slot.status" @retry="retry" /><PublicContentState v-else-if="!content?.valid" status="preparing" /><template v-else><nav class="table-of-contents" :aria-label="t('toc')"><h2>{{ t('toc') }}</h2><ol><li v-for="item in content.toc" :key="item.id"><a :href="`#${item.id}`">{{ item.text }}</a></li></ol></nav><div class="public-document__content public-richtext" v-html="content.legalBodyHTML" /><footer class="public-document__contact"><h2>{{ t('contact') }}</h2><p>{{ content.contact }}</p></footer></template></article></template>
