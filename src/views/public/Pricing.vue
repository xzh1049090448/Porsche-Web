...
let searchTimer; let disposed = false; let auth = null; let stopAuth = null; let authProbe = null
const options = () => publicPricingAuthOptions(auth)
async function upgradeAuth(required) {
  if (auth || !required) return publicPricingAuthOptions(auth).authenticated
  ...
  auth = restored; stopAuth = auth.subscribe(() => { void load() })
  return true
}
async function load() {
  await ready; if (disposed) return null
  await store.loadSite(options()); if (disposed || store.value.site.status !== 'ready') return null
  const protectedPrices = store.value.site.data?.priceVisibility === 'authenticated_only'
  if (await upgradeAuth(protectedPrices || ['input', 'output'].includes(query.value.sort))) await store.loadSite(options())
  sortRequiresLogin.value = !options().authenticated && protectedPrices && ['input', 'output'].includes(query.value.sort)
