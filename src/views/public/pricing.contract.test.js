...
test('catalog exposes desktop filters/table, mobile drawer/cards and accessible controls', async () => {
  ...
  const toolbar = renderedElements(pageTemplate, 'div').find(node => hasClass(node, 'pricing-toolbar'))
  ...
  assert.ok(toolbar && renderedElements(toolbar, 'input').some(node => staticAttribute(node, 'name') === 'search'), 'pricing toolbar must own the real search input')
  ...
  assertProperty(pricingCss, '.pricing-sidebar', 'border-radius', '10px', 'pricing rail keeps the approved compact card radius')
  ...
  assert.doesNotMatch(filters, /name\s*=\s*(["'])search\1/, 'search must not be duplicated in the desktop filter rail or mobile drawer')
  for (const field of ['provider', 'capability', 'endpoint', 'group', 'sort', 'direction']) assert.match(filters, new RegExp(`name\\s*=\\s*(["'])${field}\\1`))
  ...
  assertProperty(tableCss, '.pricing-table-wrap', 'border-radius', '10px', 'desktop results keep the approved compact card radius')
  assertProperty(tableCss, '.pricing-table-wrap', 'overflow', 'auto', 'wide result columns must scroll inside their own table card')
  ...
  assertControlSize(pricingCss, '.pricing-search input', ['min-height'], widths)
