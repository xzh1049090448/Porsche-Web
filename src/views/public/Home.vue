...
<template><div class="public-home"><PublicContentState v-if="state.value.home.status !== 'ready'" :status="state.value.home.status" @retry="load" /><PublicContentState v-else-if="!configured" status="preparing" /><template v-else>
  ...
  <section id="advantages" class="public-section" data-section="advantages" aria-labelledby="advantages-title"><h2 id="advantages-title">{{ t('advantages') }}</h2><div v-if="home.sections.advantages.length" class="public-card public-richtext" v-html="home.sections.advantages.join('')" /><PublicContentState v-else status="empty" /></section>
  <section id="models" class="public-section" data-section="models" aria-labelledby="models-title"><h2 id="models-title">{{ t('models') }}</h2><div v-if="curatedModels.length" class="public-model-wall"><RouterLink v-for="model in curatedModels" :key="model.modelKey" :to="`/pricing/${model.modelKey}`">{{ model.displayName }}<small>{{ model.provider }}</small></RouterLink></div><PublicContentState v-else :status="modelWallStatus" @retry="load" /></section>
