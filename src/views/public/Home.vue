...
const { store: state, publication, loadHome } = inject('public-home-publication'); const { t } = usePublicI18n()
...
<template><div class="public-home"><PublicContentState v-if="homeStatus !== 'ready'" :status="homeStatus" @retry="load" /><PublicContentState v-else-if="!configured" status="preparing" /><template v-else>
  <section class="public-hero" data-section="hero" aria-labelledby="home-title"><div class="public-hero__copy"><h1 id="home-title">{{ home.title }}</h1><div class="public-lead public-richtext" v-html="home.introHTML" /><div class="public-actions"><RouterLink class="public-button" to="/chat">{{ t('console') }}</RouterLink><RouterLink class="public-button public-button--secondary" to="/pricing">{{ t('pricing') }}</RouterLink></div></div><HeroPreview :label="t('demo')" /></section>
  <PublicSection id="advantages" :title="t('advantages')" data-section="advantages"><div v-if="home.advantageCards.length" class="public-benefit-grid"><article v-for="card in home.advantageCards" :key="card.title" class="public-card"><h3>{{ card.title }}</h3><div class="public-richtext" v-html="card.html" /></article></div><PublicContentState v-else status="empty" /></PublicSection>
  <PublicSection id="models" :title="t('models')" data-section="models"><div v-if="curatedEntries.length" class="public-model-wall"><RouterLink v-for="entry in curatedEntries" :key="entry.model.modelKey" :to="`/pricing/${entry.model.modelKey}`"><img v-if="entry.asset" :src="entry.asset" alt="" loading="lazy" />{{ entry.model.displayName }}<small>{{ entry.model.provider }}</small></RouterLink></div><PublicContentState v-else :status="modelWallStatus" @retry="load" /></PublicSection>
  <PublicSection :title="`${t('announcements')} / ${t('faq')}`" data-section="announcements-faq"><div class="public-info-grid"><div><h3>{{ t('announcements') }}</h3><div v-if="home.sections.announcements.length" class="public-richtext" v-html="home.sections.announcements.join('')" /><PublicContentState v-else status="empty" /></div><div><h3>{{ t('faq') }}</h3><div v-if="home.sections.faq.length" class="public-richtext" v-html="home.sections.faq.join('')" /><PublicContentState v-else status="empty" /></div></div></PublicSection>
