...
function clearPreviewState() {
  ...
  preview.value = null; revision.value = 0; loading.value = false; error.value = ''
  dynamicState.value = { status: 'idle', data: null, error: null }; featuredModels.value = null
}
function leavePreview() {
  clearPreviewState()
  void router.replace(userStore.isLoggedIn ? '/chat' : '/login')
}

async function loadPreview() {
  if (userStore.user?.role !== 'root') { leavePreview(); return }
  controller.abort(); controller = new AbortController()
  const own = ++generation, local = controller, identityEpoch = userStore.identityEpoch
  ...
  try {
    ...
    if (own !== generation || local.signal.aborted || identityEpoch !== userStore.identityEpoch || userStore.user?.role !== 'root') return
    ...
    if (own === generation && !local.signal.aborted && identityEpoch === userStore.identityEpoch && userStore.user?.role === 'root') { error.value = safeError(caught); dynamicState.value = { status: 'hidden', data: null, error: { code: error.value, requestId: null } } }
  } finally { if (own === generation && identityEpoch === userStore.identityEpoch && userStore.user?.role === 'root') loading.value = false }
}

watch([() => userStore.identityEpoch, () => userStore.user?.role], ([identityEpoch, role], [previousIdentityEpoch]) => {
  if (identityEpoch !== previousIdentityEpoch || role !== 'root') leavePreview()
}, { flush: 'sync' })
...
onMounted(() => {
  ...
  if (userStore.user?.role === 'root') void loadPreview()
  else leavePreview()
})
onBeforeUnmount(() => {
  clearPreviewState()
