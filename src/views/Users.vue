<template>
  <section class="admin-page console-page user-admin-page">
    <PageHeader ref="pageHeading" eyebrow="ADMINISTRATION" title="用户管理" description="查看当前有权访问的用户信息。" tabindex="-1">
      <template #actions><el-button v-if="canCreate" type="primary" @click="openCreate($event)">{{ t('createUser.open') }}</el-button></template>
    </PageHeader>
    ...
    <template v-else>
      <el-card shadow="never" class="filters-card filter-toolbar surface-card"><el-form class="filters" @submit.prevent="reloadFromFirstPage">
      ...
      <div class="table-surface surface-card responsive-table"><el-table v-loading="store.loading" :data="store.rows" row-key="guid" empty-text="暂无用户" class="users-table">
      ...
      <div v-loading="store.loading" class="mobile-user-cards" aria-label="用户列表">
        <article v-for="row in store.rows" :key="row.guid" class="mobile-user-card surface-card">
          <div class="mobile-user-card__heading"><strong>{{ row.username ?? '未设置' }}</strong><StatusBadge :status="statusTone(row.status)" :label="statusLabel(row.status)" /></div>
          <dl><div><dt>GUID</dt><dd><code>{{ row.guid }}</code></dd></div><div><dt>昵称</dt><dd>{{ row.nickname ?? '未设置' }}</dd></div><div><dt>角色</dt><dd>{{ roleLabel(row.role) }}</dd></div><div><dt>分组 / 套餐</dt><dd>{{ row.group }} · {{ planLabel(row.planType) }}</dd></div></dl>
          <div class="mobile-user-card__actions"><el-button link type="primary" @click="$router.push(`/users/${row.guid}`)">详情</el-button><el-button v-if="canDeleteTarget(row)" link type="danger" @click="openDelete(row, $event)">{{ t('deleteUser.confirm') }}</el-button></div>
        </article>
        <el-empty v-if="!store.loading && store.rows.length === 0" description="暂无用户" />
      </div>
      <div class="pagination-bar"><el-pagination v-model:current-page="filters.page" v-model:page-size="filters.pageSize" :page-sizes="[20, 50, 100]" layout="total, sizes, prev, pager, next" :total="store.total" @current-change="reloadForPageChange" @size-change="reloadFromFirstPage" /></div>
    </template>
...
const statusLabel = status => ({ active: '启用', disabled: '禁用', deleted: '已删除' }[status] || status); const statusTone = status => ({ active: 'active', disabled: 'warning', deleted: 'gone' }[status] || 'warning'); const roleLabel = role => ({ user: '用户', admin: '管理员', root: 'Root' }[role] || role); const planLabel = plan => ({ free: '免费版', professional: '专业版', enterprise: '企业版' }[plan] || plan)
...
<style scoped>
.admin-page{max-width:1400px;overflow:auto;height:100%}.filters-card,.error{margin-bottom:16px}.filter-toolbar{padding-bottom:8px}.filters{display:flex;flex-wrap:wrap;align-items:flex-end;gap:0 12px}.filters :deep(.el-form-item){margin-right:0}.table-surface{padding:0;overflow:hidden}.responsive-table,.users-table{width:100%}.pagination-bar{display:flex;justify-content:flex-end;margin-top:20px}.mobile-user-cards{display:none}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}code{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;overflow-wrap:anywhere}@media(max-width:768px){.filters{display:block}.filters :deep(.el-form-item){margin-bottom:12px}.responsive-table{display:none}.mobile-user-cards{display:grid;gap:12px}.mobile-user-card{padding:16px}.mobile-user-card__heading,.mobile-user-card__actions{display:flex;align-items:center;justify-content:space-between;gap:12px}.mobile-user-card dl{display:grid;gap:8px;margin:16px 0}.mobile-user-card dl div{display:grid;grid-template-columns:90px minmax(0,1fr);gap:12px}.mobile-user-card dt{color:var(--text-secondary);font-size:12px}.mobile-user-card dd{min-width:0;margin:0;text-align:right}.pagination-bar{justify-content:center;overflow-x:auto}.pagination-bar :deep(.el-pagination__sizes),.pagination-bar :deep(.el-pagination__total){display:none}}
</style>
