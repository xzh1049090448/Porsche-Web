...
test('CRUD lifecycle missing and sync use exact routes and bodies', async () => {
  ...
  await api.create({upstream_model_id:'up/m'}); await api.update('123',{expected_revision:1}); await api.activate('123',1); await api.deactivate('123',2,'retired'); await api.getMissing(); await api.sync(); await api.deleteModel('123',{expectedRevision:3,reason:'retired',ticket:'av_ticket'})
  assert.deepEqual(calls.map(c=>[c.method,c.path,c.body,c.headers]),[
    ['GET','/admin/v2/public-models',undefined,undefined],['POST','/admin/v2/public-models',{upstream_model_id:'up/m'},undefined],['PATCH','/admin/v2/public-models/123',{expected_revision:1},undefined],['POST','/admin/v2/public-models/123/activate',{expected_revision:1},undefined],['POST','/admin/v2/public-models/123/deactivate',{expected_revision:2,reason:'retired'},undefined],['GET','/admin/v2/public-models/missing',undefined,undefined],['POST','/admin/v2/public-models/sync',undefined,undefined],['DELETE','/admin/v2/public-models/123',{expected_revision:3,reason:'retired'},{'X-Action-Ticket':'av_ticket'}]
  ])
})
test('delete verification keeps password only in request and ticket out of body', async () => {
  const calls=[]; const api=createPublicModelAdminApi({request:async x=>{calls.push(x); return ok({ticket:'av_ticket',expires_at:1720000000000},201)}})
  assert.deepEqual(await api.issueDeleteVerification({guid:'123',expectedRevision:2,reason:'retired',currentPassword:'secret'}),{ticket:'av_ticket',expiresAt:1720000000000})
...
test('strict response rejects unknown fields, numeric decimals and unsafe errors', async () => {
  ...
  const conflict=createPublicModelAdminApi({request:async()=>{throw {response:{status:409,data:{error:{code:'conflict',message:'revision conflict',request_id:'req'}},headers}}}})
...
test('store cancels old reads and suppresses stale responses and delete tickets', async () => {
  ...
  const api={list:()=>new Promise(r=>{resolveFirst=r}),get:async()=>dto,getMissing:async()=>({}),sync:async()=>({accepted:true}),issueDeleteVerification:async()=>({ticket:'av_secret',expiresAt:1}),deleteModel:async()=>true}
  ...
  await c.remove({guid:'123',expectedRevision:1,reason:'retired',currentPassword:'secret'});assert.equal(JSON.stringify(state).includes('av_secret'),false)
})
