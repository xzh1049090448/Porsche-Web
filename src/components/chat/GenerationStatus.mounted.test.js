...
test('mounted lifecycle status is announced and cancellation is authoritative and idempotent', async () => {
  ...
  const store = reactive({ generationState: { status: 'disconnected', phase: 'disconnected' }, streaming: true, calls: 0, cancelStream() { this.calls += 1; this.generationState = { status: 'cancelling', phase: 'cancelling' } } })
