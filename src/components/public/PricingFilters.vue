...
const props = defineProps({ query: { type: Object, required: true }, providers: { type: Array, default: () => [] }, capabilities: { type: Array, default: () => [] } })
const emit = defineEmits(['change'])
const { t } = usePublicI18n()
const form = reactive({ ...props.query })
watch(() => props.query, value => Object.assign(form, value), { deep: true })
let timer
function change(key, value) { form[key] = value; clearTimeout(timer); timer = setTimeout(() => emit('change', { [key]: value, page: 1 }), key === 'search' ? 250 : 0) }
</script>
<template><form class="pricing-filters" :aria-label="t('pricingCatalog.filter')" @submit.prevent>
  ...
  <label>{{ t('pricingCatalog.endpoint') }}<select name="endpoint" :value="form.endpoint" @change="change('endpoint',$event.target.value)"><option value="">{{ t('pricingCatalog.allEndpoints') }}</option><option v-for="item in capabilities" :key="item" :value="item">{{ item }}</option></select></label>
  <label>{{ t('pricingCatalog.sort') }}<select name="sort" :value="form.sort" @change="change('sort',$event.target.value)"><option value="default">{{ t('pricingCatalog.defaultSort') }}</option><option value="name">{{ t('pricingCatalog.modelName') }}</option><option value="input">{{ t('pricingCatalog.inputPrice') }}</option><option value="output">{{ t('pricingCatalog.outputPrice') }}</option></select></label>
