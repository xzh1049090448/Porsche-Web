...
<template>
  ...
  <div v-else-if="status === 'error' || status === 'not_found' || status === 'gone'" class="public-state public-state--error" role="alert"><h2>{{ t('error') }}</h2><p v-if="message">{{ message }}</p><button class="public-button" type="button" @click="$emit('retry')">{{ t('retry') }}</button></div>
</template>
