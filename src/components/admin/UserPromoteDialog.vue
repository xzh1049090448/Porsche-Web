...
export function normalizePromoteDialogInput(input) {
...
const failureMessage = computed(() => ({
  ...
  action_verification_rejected: '验证被拒绝，请确认当前密码。',
  action_operation_rejected: '当前操作已被拒绝。',
  action_dependency_unavailable: '服务暂不可用，请稍后重新发起。',
  action_rate_limited: '请求过于频繁，请稍后重新发起。',
}[rolePermissionStore.failureCode] || '请求失败，请重新发起。'))
...
function normalizedInput() { return normalizePromoteDialogInput({ reason: form.reason, currentPassword: form.currentPassword, overrides: overrides.value }) }
...
function consumeInput() {
  try { return normalizedInput() } finally { clearPassword() }
}
async function submit() {
  ...
  const running = rolePermissionStore.submit(token, consumeInput)
  if (!running) return
...
<template>
  <el-dialog :model-value="visible" title="提升为管理员" width="min(720px, 94vw)" trap-focus :close-on-click-modal="false" :close-on-press-escape="true" @open="onOpen" @open-auto-focus="focusFirst" @close="requestClose" @closed="onClosed" @keydown.esc="requestClose">
    <template v-if="rolePermissionStore.target">
      ...
      <el-alert type="warning" :closable="false" show-icon title="提升会使目标用户的全部现有会话立即失效，需要重新登录。" />
    ...
    <template #footer><el-button @click="requestClose">取消</el-button><el-button type="primary" native-type="submit" form="admin-user-promote-form" :loading="busy" :disabled="!canSubmit">确认提升</el-button></template>
  </el-dialog>
</template>

<style scoped>
.permission-toggle { margin-bottom: 16px; }
p { margin: 12px 0; }
@media (max-width: 720px) { .permission-toggle { width: 100%; } }
</style>
