  ...
  >
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-position="top" scroll-to-error @submit.prevent="submit">
      ...
      <el-form-item prop="nickname" :label="t('createUser.nickname')">
        <el-input v-model="form.nickname" autocomplete="off" maxlength="64" :disabled="busy" />
      </el-form-item>
...
const busy = computed(() => isAdminUserCreateBusy(createStore.state))
const canSubmit = computed(() => canSubmitAdminUserCreate({ state: createStore.state, role: form.role, catalog: createStore.catalog }))
...
function focusUsername() {
  const token = createStore.captureOwnership()
  if (!token) return
  nextTick(() => { if (createStore.owns(token)) usernameInput.value?.focus?.() })
}
...
async function submit() {
  ...
  try { await formRef.value?.validate?.() } catch {
    if (createStore.owns(token)) nextTick(() => { if (createStore.owns(token)) formRef.value?.scrollToField?.('username') })
    return
  }
