...
export function normalizeDemoteDialogInput(input) {
...
const validating = ref(false); const form = reactive({ reason: '', currentPassword: '' })
let visibleOwner = null; let closingOwner = null
const visible = computed(() => rolePermissionStore.isOpen && rolePermissionStore.action === 'users.demote' && rolePermissionStore.owns(props.owner))
const busy = computed(() => validating.value || ['verifying', 'executing', 'querying'].includes(rolePermissionStore.phase))
const canSubmit = computed(() => Boolean(rolePermissionStore.target) && rolePermissionStore.phase === 'idle' && !busy.value)
...
function clearForm() { clearPassword(); form.reason = ''; formRef.value?.clearValidate?.() }
function normalizedInput() { return normalizeDemoteDialogInput({ reason: form.reason, currentPassword: form.currentPassword }) }
...
<template>
  <el-dialog :model-value="visible" title="降级为普通用户" width="min(560px, 94vw)" trap-focus :close-on-click-modal="false" :close-on-press-escape="true" @open="onOpen" @open-auto-focus="focusFirst" @close="requestClose" @closed="onClosed" @keydown.esc="requestClose">
    <template v-if="rolePermissionStore.target">
      ...
      <el-alert type="warning" :closable="false" show-icon title="降级后管理员权限停止生效，目标用户的全部现有会话立即失效。" />
      <el-form id="admin-user-demote-form" ref="formRef" :model="form" :rules="rules" label-position="top" scroll-to-error @submit.prevent="submit">
    ...
    <template #footer><el-button @click="requestClose">取消</el-button><el-button type="danger" native-type="submit" form="admin-user-demote-form" :loading="busy" :disabled="!canSubmit">确认降级</el-button></template>
  </el-dialog>
</template>
