...
const editorStub=`data:text/javascript;base64,${Buffer.from("export function buildPermissionEditorModel(){return {valid:true}};export default {name:'UserPermissionEditor',props:['modelValue','catalog','policy'],emits:['update:modelValue'],setup(){return()=>null}}").toString('base64')}`
async function loadDialog(name){
  ...
  const replacements=new Map([['vue',new URL('../../../node_modules/vue/index.mjs',import.meta.url).href],['../../stores/admin-user-role-permissions.js',new URL('../../stores/admin-user-role-permissions.js',import.meta.url).href],['../../api/admin-user-roles-permissions.js',new URL('../../api/admin-user-roles-permissions.js',import.meta.url).href],['./UserPermissionEditor.vue',editorStub]])
...
test('promotion starts at baseline and mounts its controlled editor only after disclosure',async()=>{let state;try{state=await harness(configs[0]);assert.equal(state.wrapper.findComponent({name:'UserPermissionEditor'}).exists(),false);await state.wrapper.get('.permission-toggle').trigger('click');await flush();assert.equal(state.wrapper.findComponent({name:'UserPermissionEditor'}).exists(),true)}finally{cleanup(state)}})
