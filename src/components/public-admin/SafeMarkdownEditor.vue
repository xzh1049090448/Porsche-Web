<template><SurfaceCard class="editor"><label :for="id">{{ label }}</label><textarea :id="id" :value="modelValue" :aria-describedby="`${id}-help`" @input="$emit('update:modelValue',$event.target.value)"/><p :id="`${id}-help`">{{ help }}</p><div class="preview" aria-live="polite" v-html="preview" /></SurfaceCard></template>
<script setup>
