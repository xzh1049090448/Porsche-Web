...
const raw=(row,side,field)=>{const model=side==='draft'?row.draftModel:row.liveModel;if(!model)return MISSING;const value=model[field[side]];if(field.instant)return canonicalPricingInstant(value);return field.array?[...value].sort():value}
