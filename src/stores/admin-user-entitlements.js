...
export function createDirectEntitlementCoordinator({state,canOpen,createWorkflow,execute,onSucceeded=()=>{}}={}){
  ...
  let owner=null,context=null,workflow=null,unsubscribe=null,active=null,conflictFlight=null,sequence=0
  ...
  const destroy=()=>{unsubscribe?.();unsubscribe=null;workflow?.dispose();workflow=null;owner=null;context=null;active=null;conflictFlight=null}
  ...
  const refreshConflict=(token,load)=>{if(!current(token)||state.state!=='conflict'||typeof load!=='function')return Promise.resolve(false);if(conflictFlight?.token===token&&conflictFlight.context===context)return conflictFlight.promise;const captured=context,flight={token,context:captured,promise:null};conflictFlight=flight;flight.promise=(async()=>{let target;try{target=await load(captured.target.guid)}catch{return false}if(conflictFlight!==flight||!current(token)||context!==captured||!canOpen({...captured,target})||target.guid!==captured.target.guid)return false;context=Object.freeze({...captured,target:Object.freeze({...target})});state.target=context.target;state.requiresTargetRefresh=false;workflow.reset();return true})().finally(()=>{if(conflictFlight===flight)conflictFlight=null});return flight.promise}
