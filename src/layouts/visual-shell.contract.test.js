...
test('Task 4 builds an accessible responsive public shell from a dedicated stylesheet', async () => {
  ...
  for (const route of ['/about', '/terms', '/privacy']) assert.match(footer, new RegExp(`link\\('${route}'`))
  assert.match(footer, /h\(RouterLink,\s*\{ to \}/)
  assert.match(footer, /h\('a',\s*\{ href: item\.href \}/)
...
test('public mobile navigation closes for route changes, Escape, and desktop breakpoints', async () => {
  ...
  try {
    ...
    try {
      ...
      const consoleCtas = wrapper.element.querySelectorAll('.public-console-cta')
      assert.equal(consoleCtas.length, 1)
      assert.equal(consoleCtas[0].closest('#public-mobile-nav')?.id, 'public-mobile-nav')
      assert.equal(consoleCtas[0].getAttribute('href'), '/chat')
