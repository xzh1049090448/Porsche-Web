...
export function readPublicLocale(storage = globalThis.localStorage) { return getItem(PUBLIC_LOCALE_KEY, 'zh', storage) === 'en' ? 'en' : 'zh' }
...
export function persistPublicLocale(locale, storage = globalThis.localStorage, target = globalThis.document) { const value = locale === 'en' ? 'en' : 'zh'; setItem(PUBLIC_LOCALE_KEY, value, storage); applyPublicLocale(value, target); return value }
