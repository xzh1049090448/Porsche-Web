...
发布载荷不保存原始草稿 Markdown、删除项、上游原始响应或凭据。legacy `home` 只保存服务端净化后的兼容 HTML，固定首页不读取它。`content_hash` 对规范化后的完整载荷计算 SHA-256；数组顺序、对象键顺序、空值表示和时间格式必须由服务端唯一确定。
...
- 现有 `/api/v1/public/home` 继续从发布快照返回已净化的 legacy 文档，供旧客户端兼容；新首页不调用它。
...
**用户设计确认：** 2026-09-15，已确认架构、数据接口、后台流程、异常处理与验收四部分，并确认本文件进入实施计划阶段。
